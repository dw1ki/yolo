...
async def upload_to_cloudinary(file: UploadFile):
    ...
    # Return both temp path and Cloudinary URL
    return {
        "tmp_path": tmp_path,
...
async def process_video(job_id: str, video_url: str):
    ...
    try:
        ...
        print(f"   [DEBUG] cv2.VideoCapture created")
        
        if not cap.isOpened():
            print(f"   ❌ Cannot open video from URL: {video_url}")
        ...
        # Use FPS and frame count to calculate duration (CAP_PROP_DURATION may not work with URLs)
        ...
        print(f"   [DEBUG] Video: FPS={fps}, Frames={total_frames}, Duration={duration:.1f}s")
        ...
        # Re-get total_frames in case it changed after validation
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        if total_frames <= 0:
            total_frames = 1
        print(f"   [DEBUG] Processing {total_frames} frames")
        ...
        while True:
            ...
            if frame is None or frame.size == 0:
                print(f"   [WARN] Frame {frame_count} is empty, skipping")
                frame_count += 1
                continue
            ...
            try:
                ...
                print(f"   [WARN] Failed to resize frame {frame_count}: {e}")
            ...
            try:
                ...
                print(f"   [WARN] Failed to predict on frame {frame_count}: {e}")
                continue  # Skip this frame
        ...
        print(f"🟢 [process_video END] Job {job_id} - Status: {jobs[job_id]['status']}\n")
        
        # Clean up temp video file
        try:
            if os.path.exists(video_url):
                os.remove(video_url)
                print(f"   Cleaned up temp file: {video_url}")
        except Exception as e:
            print(f"   [WARN] Failed to clean up temp file: {e}")
    
    except Exception as e:
        print(f"🔴 [process_video ERROR] Job {job_id}: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
...
async def detect_video(
    ...
    print(f"[DEBUG] Video saved to temp: {video_path}")
    print(f"[DEBUG] Video also uploaded to Cloudinary: {video_url}")
    job_id = str(int(asyncio.get_event_loop().time() * 1000))
    print(f"[DEBUG] Created job_id: {job_id}")
    
    # Create task to run process_video in background
    # Use create_task to properly schedule async function
    try:
        asyncio.create_task(process_video(job_id, video_path))
        print(f"[DEBUG] Started background task for job {job_id}")
    except Exception as e:
